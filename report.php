...
switch($code){
  ...
  $result =mysql_query("select * from distress where  status=1 order by landlord");
    ...
    $result =mysql_query("select * from distress  where stamp>='".$d1."' and stamp<='".$d2."'  and status=1 order by distress");
  }
...
$aa=$i+1;
  ...
  <td  style="width:15%;border-width:0.5px; border-color:#666; border-style:solid; "><?php  echo stripslashes($rowa['at']) ?></td>
  <td  style="width:8%;border-width:0.5px; border-color:#666; border-style:solid; "><?php  echo stripslashes($rowa['date']) ?></td>
