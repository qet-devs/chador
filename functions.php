...
function getledgerbreakdown($param, $sent, $level, $d1, $d2)
{
    ...
    for ($i = 1; $i < $max; $i++) {
        ...
        if ($catid == $param) {
            ...
            ?>
            <td style="width:10%;border-width:0.2px; border-color:#ccc; border-style:solid;text-align:left;"><span
                        style="margin-left:<?php echo $margin ?>%">
            <img src="img/bullet.png" style="width:12px; height:12px; margin:0 3px -2px 0"><?php echo $name ?></span>
            </td>
...
function showbreakdown($lid, $array, $d1, $d2)
{
    ...
    if (count($array)) {
        ...
        foreach ($array as $item) {
            ...
            $sent = '';
            if ($d1 != 0) {
                $sent .= '&d1=' . stamptodate($d1);
            }
            if ($d2 != 0) {
                $sent .= '&d2=' . stamptodate($d2);
            }

            if (isset($_SESSION['setledgers'][$item['ledgerid']])) {
                $status = 1;
            } else {
                $status = 0;
                $_SESSION['setledgers'][$item['ledgerid']] = 0;
            }
            ...
            if ($lid != $item['ledgerid'] && $status == 0) {
                ...
                if ($type == 'Expense' || $type == 'Asset') {
                    echo number_format(floatval($_SESSION['allledgers'][$item['ledgerid']]), 2, ".", ",");
                }
                echo '</td>
                            <td style="width:25%;border-width:0.5px; border-color:#666; border-style:solid;text-align:right">';
                if ($type == 'Liability' || $type == 'Revenue' || $type == 'Equity') {
...
function printledgerlist($array, $d1, $d2)
{
    ...
    if (count($array)) {
        ...
        foreach ($array as $item) {
            ...
            if ($type == 'Expense' || $type == 'Asset') {
                $bal = $dr - $cr;
                $yearbal = $drmain - $crmain;
            }
            if ($type == 'Liability' || $type == 'Revenue' || $type == 'Equity') {
                $bal = $cr - $dr;
                $yearbal = $crmain - $drmain;
            }
...
function cartitems($max)
{
    ...
    for ($row = 0; $row < $max; $row++) {
        ...
        if ($row % 2 == 0) {
            echo '<tr style="width:100%; height:20px;padding:0; font-weight:normal;cursor:pointer ">';
        } else {
...
function cartjournal($max)
{
    ...
    for ($row = 0; $row < $max; $row++) {
        $subid = '';
        if ($_SESSION['journal'][$row][4] != '') {
            $subid = '[' . $_SESSION['journal'][$row][4] . ']';
        }
        ...
        echo '<td style="width:45%;border-width:0.5px; border-color:#666; border-style:solid;">' . $_SESSION['journal'][$row][1] . $subid . '</td>
                                       <td style="width:20%;border-width:0.5px; border-color:#666; border-style:solid;">';
        if ($_SESSION['journal'][$row][2] == 'Debit') {
            echo number_format($_SESSION['journal'][$row][3]);
            $aa += $_SESSION['journal'][$row][3];
        }
        echo '</td>
                                       <td style="width:20%;border-width:0.5px; border-color:#666; border-style:solid;">';
        if ($_SESSION['journal'][$row][2] == 'Credit') {
            echo number_format($_SESSION['journal'][$row][3]);
            $bb += $_SESSION['journal'][$row][3];
        }
        echo '</td>
