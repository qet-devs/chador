...
<!-- Mirrored from vendroid.venmond.com/pages-login.php by HTTrack Website Copier/3.x [XR&CO'2013], Wed, 18 Feb 2015 17:44:49 GMT -->
                ...
                <div class="panel-body" style="border:1px solid #ddd">
                  ...
                  <form class="form-horizontal" id="login-form" action="#" role="form">
                    <div class="form-group  mgbt-xs-20">
                      <div class="col-md-12">
                         <div class="label-wrapper sr-only"  style="display:none">
                    ...
                    <div class="form-group">
                      <div class="col-md-12 text-center mgbt-xs-5">
                        <button class="btn vd_bg-green vd_white width-100" type="button" id="login-submit" onclick="login()">Login  <span  id="message"></span></button>
                         <div  id="message"></div>
                        <small ><a style="cursor:pointer;margin-top:20px" class="text-center" onclick="window.open('http://www.qet.co.ke')">&copy; QET SYSTEMS 2011-<?php echo date('Y') ?></a></small>
                       </div>
      ...
      function keydown(evt){
        if (!evt) evt = event;
           if (evt.keyCode==13){ //enter
               if($('#username').val!=''&& $('#password').val()!='' && $('#property').val()!=''){
                    login();
               }else{
                $('#password').focus();
               }
...
function login(){
  ...
  var property=$('#property').val();
    ...
    swal("Error", "Enter your Password!", "error");
     ...
     $('#message').html('<img id="img-spinner" src="img/spin.gif" style="margin-top:0px;width:20px" alt="Loading"/>');
        $.ajax({
        url:'login.php',
        data:{username:username, passwd:passwd,property:property},
