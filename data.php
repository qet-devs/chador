...
switch ($id) {
    ...
    case 13:
        ...
        if ($result) {
            ...
            echo '<img src="img/delete.png" style="margin-top:0px;width:30px; height:30px">';
    ...
    case 14:
        ...
        if ($a == 2) {
            ...
            if ($housestatus == 'Not for Rent') {
                $resultg = mysql_query("update houses set status='1' where rid='" . $rid . "'") or die (mysql_error());
            }
    ...
    case 15:
        ...
        if ($resulta) {
            ...
            if (stripslashes($rowa['Admin']) == 'YES') {
                $arr[] = 'Admin';
            }
            if (stripslashes($rowa['Accounts']) == 'YES') {
                $arr[] = 'Accounts';
            }
            if (stripslashes($rowa['Procurement']) == 'YES') {
                $arr[] = 'Procurement';
            }
    ...
    case 16:
        ...
        if ($resulta) {
            ...
            echo '<img src="img/delete.png" style=""  width="20" height="20"/>';
        }
    ...
    case 23:
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            //insert invoice
            if ($fintot >= 0) {
                $desc .= $actname . '-INVOICE FOR ' . $month . ';';
            } else {
                $desc .= $actname . '-CREDIT NOTE:' . $month . ';';
            }
    ...
    case 25:
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            if ($lid == 628) {
                ...
                if ($vatstatus == 1) {
                    $vat = round((($tenfigure / (1 + $vatper)) * $vatper), 2);
                } else {
                    $vat = 0;
                }
    ...
    case 52:
        ...
        $description = 'Supplier Payment:' . $supname . '-' . $supid . '-Ref No:' . $refno;
        $refno = $refno;
    ...
    case 53:
        ...
        for ($i = 0; $i < $num_results; $i++) {
            ...
            if ($vatstatus == 1) {
                $vat = round((($amount / (1 + $vatper)) * $vatper), 2);
            } else {
                $vat = 0;
            }
    ...
    case 62:
        ...
        foreach ($_SESSION['recipients'] as $key => $val) {
            $categ = substr($key, 0, 1);
            $len = strlen($key);
            $len = $len - 1;
            $param = substr($key, 1, $len);
            if ($categ == 1) {
                $tid = $param;
                ...
                $phone = stripslashes($row['phone']);
                $phonearr = explode('/', ($phone));
                foreach ($phonearr as $key2 => $val2) {
                    $sms[$val2] = '1' . $tid;
                }
                $email = stripslashes($row['email']);
                $emailarr = explode('/', ($email));
                foreach ($emailarr as $key2 => $val2) {
                    $mail[$val2] = '1' . $tid;
                }
        ...
        if ($checksms == 1) {
            foreach ($sms as $key => $val) {
                $categ = substr($val, 0, 1);
                $len = strlen($val);
                $len = $len - 1;
                $param = substr($val, 1, $len);
                if ($categ == 1) {
    ...
    case 67:
        ...
        $details = $description = 'Credit Note:' . $tname . '-' . $tid . '-Details:' . $refno;
        $refno = $refno;
    ...
    case 77:
        ...
        $details = $description = 'Refund Note:' . $tname . '-' . $tid . '-Details:' . $refno;
        $refno = $refno;
    ...
    case 80:
        ...
        for ($i = 0; $i < $num_results; $i++) {
            ...
            if (stripslashes($row['housestatus']) == 'Not for Rent') {
                $amount = 0;
                $tenant = 'NOT FOR RENT [' . $tenant . ']';
            }
    ...
    case 98:
        ...
        if ($result) {
            echo '<img src="img/tick.png" style=""  width="30" height="30"/>';
            if ($a == 1) {
                echo "<script>setTimeout(function() {workregistry();},500);</script>";
            } else if ($a == 2) {
    ...
    case 101:
        ...
        if ($result) {
            echo '<img src="img/tick.png" style=""  width="30" height="30"/>';
            if ($a == 1) {
                echo "<script>setTimeout(function() {forumsregistry();},500);</script>";
            } else if ($a == 2) {
    ...
    case 103:
        ...
        if ($result) {
            echo '<img src="img/tick.png" style=""  width="30" height="30"/>';
            if ($a == 1) {
                echo "<script>setTimeout(function() {cmeregistry();},500);</script>";
            } else if ($a == 2) {
    ...
    // all CASE MANAGEMENT data logic goes here
//save new case management
    case 300:
        ...
        $upload_file_no = $_GET['upload_file_no'];
        $unique_file_id = generateUniqueFileID($case_type);
        $resultx = mysql_query('SELECT * FROM `case_files` WHERE `client_uid`="' . $client_uid . '" and `referrer_uid`="' . $referrer_uid . '" and `description`="' . $description . '" ');
        if (mysql_num_rows($resultx) > 0) {
            echo '<script>swal("Error", "Case file with similar info already exists. !Consult the System Admin", "error");</script>';
...
//        update case management info
    ...
    case 304:
        ...
        $result = mysql_query("select * from clients where  id='" . $clientid . "'  limit 0,1");
        $row = mysql_fetch_array($result);
...
// check invoice to ensure its not 0
    ...
    case 400:
        ...
        if (mysql_num_rows($resultx) > 0) {
            echo '<script>swal("Error", "warrant file with similar info already exists. !Consult the System Admin", "error");</script>';
    ...
    case 401:
        ...
        if ($resultc) {
            ...
            echo '<script>swal("Error", "failed to update Warrant record!", "error");</script>';
        }
        break;
    //end edit warrant
